...
DEFAULT_CONFIG_PATH = CLEANROOM_DIR / "configs" / "ordinal_cumulative_current_head_monotonic_120min_hard_soft_threshold_weighted_inner_identity_dedup.yaml"


def build_candidate_grid(config: dict[str, object]) -> list[dict[str, object]]:
    objective_cfg = config["training_objective"]
    candidates = [{"candidate_name": "default", "hard_weight": 1.0, "peak_weight": 1.0, "radius": None}]
    for hard_weight in objective_cfg["hard_weight_grid"]:
        for peak_weight in objective_cfg["peak_weight_grid"]:
            hard_weight = float(hard_weight)
            peak_weight = float(peak_weight)
            if peak_weight < hard_weight:
                continue
            for radius in objective_cfg["radius_grid"]:
                radius = float(radius)
                candidates.append(
                    {
                        "candidate_name": f"hard_{hard_weight:.2f}_peak_{peak_weight:.2f}_radius_{radius:.2f}",
...
def build_hard_soft_sample_weight(
    train_frame: pd.DataFrame,
    threshold: float,
    candidate: dict[str, object],
) -> pd.Series | None:
    hard_weight = float(candidate["hard_weight"])
    peak_weight = float(candidate["peak_weight"])
    radius = candidate["radius"]
    if hard_weight == 1.0 and (radius is None or peak_weight == 1.0):
        return None

    weights = pd.Series(1.0, index=train_frame.index, dtype=float)
    if hard_weight != 1.0:
        weights.loc[train_frame["boundary_any_flag"]] = hard_weight

    if radius is not None and peak_weight > 1.0:
        radius = float(radius)
        distance = (train_frame["t90"] - float(threshold)).abs()
        closeness = (1.0 - (distance / radius)).clip(lower=0.0, upper=1.0)
        soft_weights = 1.0 + (peak_weight - 1.0) * closeness
        weights = pd.Series(
            [max(left, right) for left, right in zip(weights.tolist(), soft_weights.tolist())],
...
def fit_cumulative_business_probabilities(
    ...
    *,
    ...
    for threshold in thresholds:
        ...
        sample_weight = build_hard_soft_sample_weight(train_frame, float(threshold), candidate) if float(threshold) in weighted_thresholds else None
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Test hard-soft mixed threshold weighting on the strongest identity-deduped simple baseline.")
    ...
    for threshold in thresholds:
        ...
        predictions[f"mixed_cum_prob_lt_{key}"] = pd.NA
    for label in BUSINESS_LABELS:
        predictions[f"default_business_prob_{label}"] = pd.NA
        predictions[f"mixed_business_prob_{label}"] = pd.NA
    predictions["default_business_pred"] = pd.NA
    predictions["mixed_business_pred"] = pd.NA
    ...
    mixed_eval_rows: list[dict[str, object]] = []
    default_candidate = {"candidate_name": "default", "hard_weight": 1.0, "peak_weight": 1.0, "radius": None}
    ...
    for fold_index, (train_index, test_index) in enumerate(outer_tscv.split(feature_rows), start=1):
        ...
        mixed_cumulative, mixed_probabilities = fit_cumulative_business_probabilities(
            train_frame,
            test_frame,
            features=selected_features,
            thresholds=thresholds,
        ...
        mixed_pred = probability_argmax(mixed_probabilities, BUSINESS_LABELS)

        for threshold in thresholds:
            key = format_threshold(threshold)
            predictions.loc[test_frame.index, f"default_cum_prob_lt_{key}"] = default_cumulative[f"lt_{key}"]
            predictions.loc[test_frame.index, f"mixed_cum_prob_lt_{key}"] = mixed_cumulative[f"lt_{key}"]
        for label in BUSINESS_LABELS:
            predictions.loc[test_frame.index, f"default_business_prob_{label}"] = default_probabilities[label]
            predictions.loc[test_frame.index, f"mixed_business_prob_{label}"] = mixed_probabilities[label]
        predictions.loc[test_frame.index, "default_business_pred"] = default_pred
        predictions.loc[test_frame.index, "mixed_business_pred"] = mixed_pred

        default_metrics = evaluate_prediction_set(test_frame, default_probabilities, default_pred)
        mixed_metrics = evaluate_prediction_set(test_frame, mixed_probabilities, mixed_pred)
        ...
        mixed_eval_rows.append(
            {
                "fold": fold_index,
                "macro_f1": mixed_metrics["macro_f1"],
                "balanced_accuracy": mixed_metrics["balanced_accuracy"],
                "core_qualified_average_precision": mixed_metrics["core_qualified_average_precision"],
                "boundary_warning_average_precision": mixed_metrics["boundary_warning_average_precision"],
                "clearly_unacceptable_average_precision": mixed_metrics["clearly_unacceptable_average_precision"],
                "boundary_high_confidence_non_warning_rate": mixed_metrics["boundary_overconfidence"]["high_confidence_non_warning_rate"],
            }
        )
        ...
        per_fold_rows.append(
            {
                ...
                "chosen_hard_weight": chosen_candidate["hard_weight"],
                ...
                "mixed_macro_f1": mixed_metrics["macro_f1"],
                "default_balanced_accuracy": default_metrics["balanced_accuracy"],
                "mixed_balanced_accuracy": mixed_metrics["balanced_accuracy"],
                "default_boundary_high_conf_non_warning_rate": default_metrics["boundary_overconfidence"]["high_confidence_non_warning_rate"],
                "mixed_boundary_high_conf_non_warning_rate": mixed_metrics["boundary_overconfidence"]["high_confidence_non_warning_rate"],
            }
        )

    for label in BUSINESS_LABELS:
        predictions[f"default_business_prob_{label}"] = pd.to_numeric(predictions[f"default_business_prob_{label}"], errors="coerce")
        predictions[f"mixed_business_prob_{label}"] = pd.to_numeric(predictions[f"mixed_business_prob_{label}"], errors="coerce")
    for threshold in thresholds:
        key = format_threshold(threshold)
        predictions[f"default_cum_prob_lt_{key}"] = pd.to_numeric(predictions[f"default_cum_prob_lt_{key}"], errors="coerce")
        predictions[f"mixed_cum_prob_lt_{key}"] = pd.to_numeric(predictions[f"mixed_cum_prob_lt_{key}"], errors="coerce")

    scored_predictions = predictions.loc[
        predictions["default_business_pred"].notna() & predictions["mixed_business_pred"].notna()
    ].copy()
    ...
    mixed_summary = evaluate_predictions(
        scored_predictions,
        pred_col="mixed_business_pred",
        acceptable_prob_col="mixed_business_prob_acceptable",
        warning_prob_col="mixed_business_prob_warning",
        unacceptable_prob_col="mixed_business_prob_unacceptable",
    )
    ...
    mixed_monotonicity = monotonicity_summary(
        scored_predictions.rename(columns={f"mixed_cum_prob_lt_{format_threshold(t)}": f"cum_prob_lt_{format_threshold(t)}" for t in thresholds}),
        thresholds,
    )
    default_fold_mean = aggregate_metrics([{k: v for k, v in row.items() if k != "fold"} for row in default_eval_rows])
    mixed_fold_mean = aggregate_metrics([{k: v for k, v in row.items() if k != "fold"} for row in mixed_eval_rows])
    improvement = {
        "macro_f1_delta": mixed_summary["macro_f1"] - default_summary["macro_f1"],
        "balanced_accuracy_delta": mixed_summary["balanced_accuracy"] - default_summary["balanced_accuracy"],
        "core_qualified_average_precision_delta": mixed_summary["core_qualified_average_precision"] - default_summary["core_qualified_average_precision"],
        "boundary_warning_average_precision_delta": mixed_summary["boundary_warning_average_precision"] - default_summary["boundary_warning_average_precision"],
        "clearly_unacceptable_average_precision_delta": mixed_summary["clearly_unacceptable_average_precision"] - default_summary["clearly_unacceptable_average_precision"],
        "boundary_high_confidence_non_warning_rate_delta": mixed_summary["boundary_overconfidence"]["high_confidence_non_warning_rate"] - default_summary["boundary_overconfidence"]["high_confidence_non_warning_rate"],
    }

    per_fold_csv = artifacts_dir / "hard_soft_threshold_weighted_per_fold.csv"
    selected_candidate_csv = artifacts_dir / "hard_soft_threshold_weighted_selected_candidate_per_fold.csv"
    inner_search_csv = artifacts_dir / "hard_soft_threshold_weighted_inner_search.csv"
    results_csv = artifacts_dir / "hard_soft_threshold_weighted_results.csv"
    alias_csv = artifacts_dir / "sensor_identity_alias_pairs.csv"
    summary_json = artifacts_dir / "hard_soft_threshold_weighted_summary.json"
    report_md = reports_dir / "hard_soft_threshold_weighted_summary.md"
    ...
    summary = {
        ...
        "mixed_monotonicity": mixed_monotonicity,
        "default_fold_mean": default_fold_mean,
        "mixed_fold_mean": mixed_fold_mean,
    ...
    lines = [
        "# Hard-Soft Threshold-Weighted Summary",
        "",
        ...
        "- Intervention: mixed hard boundary uplift plus soft distance-to-threshold uplift on inner thresholds only",
        f"- Candidate grid size: {len(candidates)}",
        f"- Scored rows: {len(scored_predictions)}",
        f"- Default macro_f1: {default_summary['macro_f1']:.4f}",
        f"- Mixed macro_f1: {mixed_summary['macro_f1']:.4f}",
        f"- Default balanced_accuracy: {default_summary['balanced_accuracy']:.4f}",
        f"- Mixed balanced_accuracy: {mixed_summary['balanced_accuracy']:.4f}",
        f"- Default core_AP: {default_summary['core_qualified_average_precision']:.4f}",
        f"- Mixed core_AP: {mixed_summary['core_qualified_average_precision']:.4f}",
        f"- Default warning_AP: {default_summary['boundary_warning_average_precision']:.4f}",
        f"- Mixed warning_AP: {mixed_summary['boundary_warning_average_precision']:.4f}",
        f"- Default unacceptable_AP: {default_summary['clearly_unacceptable_average_precision']:.4f}",
        f"- Mixed unacceptable_AP: {mixed_summary['clearly_unacceptable_average_precision']:.4f}",
        ...
        f"- Mixed boundary high-confidence non-warning: {mixed_summary['boundary_overconfidence']['high_confidence_non_warning_rate']:.4f}",
        "",
    ...
    for row in selected_candidate_rows:
        lines.append(
            ...
            f"(hard_weight={row['hard_weight']}, peak_weight={row['peak_weight']}, radius={row['radius']})"
        )

...
DEFAULT_CONFIG_PATH = (
    CLEANROOM_DIR / "configs" / "distributional_conformal_reject_ordered_interval_boundary_selector_current_head.yaml"
)
...
def select_alpha_on_calibration(
    ...
    selector_cfg = conformal_cfg["selector"]
    ...
    for alpha in [float(item) for item in conformal_cfg["alpha_grid"]]:
        scores = [
            ordered_interval_score(probability_matrix[idx], true_indices[idx])
            for idx in range(len(calibration_frame))
        ]
        ...
        score = (
            float(selector_cfg["boundary_focus_weight"]) * (metrics["boundary_retest_rate"] - metrics["retest_rate"])
            + float(selector_cfg["decision_coverage_weight"]) * metrics["decision_coverage"]
            + float(selector_cfg["hard_bin_coverage_weight"]) * metrics["hard_bin_coverage"]
            - float(selector_cfg["retest_rate_penalty"]) * metrics["retest_rate"]
            - float(selector_cfg["mean_set_size_penalty"]) * (metrics["mean_set_size"] - 1.0)
        )
    ...
    chosen = eligible.sort_values(
        ["selector_score", "decision_coverage", "hard_bin_coverage", "mean_set_size"],
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run ordered-interval conformal with boundary-aware alpha selector."
    )
    ...
    summary = {
        ...
        "conformal_design": {
            ...
            "method": "ordered_interval_quantile_boundary_selector",
    ...
    lines = [
        ...
        "",
        "- Branch stage: conformal_ordered_interval_boundary_selector",
        "- Target: soft 5-bin distribution + ordered interval set + boundary-aware alpha selector",

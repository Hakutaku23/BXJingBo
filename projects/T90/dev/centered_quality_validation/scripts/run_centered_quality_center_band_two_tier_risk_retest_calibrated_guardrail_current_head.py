...
from run_centered_quality_center_band_risk_guardrail_current_head import (  # noqa: E402
    guardrail_decision,
...
DEFAULT_CONFIG_PATH = CENTERED_DIR / "configs" / "centered_quality_center_band_two_tier_risk_retest_calibrated_guardrail_current_head.yaml"
    ...
    *,
    ...
    for candidate in candidates:
        ...
        for inner_train_index, inner_val_index in inner_tscv.split(train_frame):
            ...
            clear_risk = pd.concat([clear_lower.astype(float), clear_upper.astype(float)], axis=1).max(axis=1)
            retest_risk = pd.concat([retest_lower.astype(float), retest_upper.astype(float)], axis=1).max(axis=1)
            decision = guardrail_decision(clear_risk, retest_risk, center_prob, candidate)
...
def write_summary_report(path: Path, summary: dict[str, object]) -> None:
    lines = [
        "# Center-Band Two-Tier Risk Retest-Calibrated Guardrail Summary",
        "",
        "## Main outcome",
        "",
...
def main() -> None:
    ...
    for fold, (train_index, test_index) in enumerate(outer_tscv.split(feature_rows), start=1):
        ...
        clear_risk = pd.concat([clear_lower.astype(float), clear_upper.astype(float)], axis=1).max(axis=1)
        retest_risk = pd.concat([retest_lower.astype(float), retest_upper.astype(float)], axis=1).max(axis=1)
        decision = guardrail_decision(clear_risk, retest_risk, center_prob, chosen_semantic_candidate)
    ...
    feature_rows.to_csv(artifacts_dir / "centered_quality_center_band_two_tier_risk_retest_calibrated_guardrail_feature_rows.csv", index=False, encoding="utf-8-sig")
    scored_predictions.to_csv(artifacts_dir / "centered_quality_center_band_two_tier_risk_retest_calibrated_guardrail_results.csv", index=False, encoding="utf-8-sig")
    per_fold_frame.to_csv(artifacts_dir / "centered_quality_center_band_two_tier_risk_retest_calibrated_guardrail_per_fold.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame(baseline_candidate_rows).to_csv(artifacts_dir / "centered_quality_center_band_two_tier_risk_retest_calibrated_guardrail_baseline_candidate_search.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame(semantic_candidate_rows).to_csv(artifacts_dir / "centered_quality_center_band_two_tier_risk_retest_calibrated_guardrail_semantic_candidate_search.csv", index=False, encoding="utf-8-sig")
    alias_frame.to_csv(artifacts_dir / "sensor_identity_alias_pairs.csv", index=False, encoding="utf-8-sig")
    (artifacts_dir / "centered_quality_center_band_two_tier_risk_retest_calibrated_guardrail_summary.json").write_text(json.dumps(json_ready(summary), ensure_ascii=False, indent=2), encoding="utf-8")
    write_summary_report(reports_dir / "centered_quality_center_band_two_tier_risk_retest_calibrated_guardrail_summary.md", json_ready(summary))

...
from run_distributional_conformal_reject_ordered_interval_boundary_selector_current_head import (
    select_alpha_on_calibration,
)
from run_distributional_conformal_reject_ordered_interval_current_head import (
    ordered_interval_set,
)
...
DEFAULT_CONFIG_PATH = (
    CLEANROOM_DIR
    / "configs"
    / "distributional_conformal_reject_ordered_interval_boundary_selector_warning_guardrail_current_head.yaml"
)
...
def operational_decision_with_warning_guardrail(bin_set: list[str]) -> str:
    business_set = set(business_set_from_bin_set(bin_set))
    if len(business_set) == 1:
        return next(iter(business_set))
    if business_set in ({"acceptable", "warning"}, {"warning", "unacceptable"}):
        return "warning"
    return RETEST_LABEL
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run ordered interval + boundary-aware alpha selector + warning-guardrail business mapping."
    )
    ...
    for fold_index, (train_index, test_index) in enumerate(outer_tscv.split(feature_rows), start=1):
        ...
        test_operational_decisions = [operational_decision_with_warning_guardrail(item) for item in test_prediction_sets]
        ...
        predictions.loc[test_frame.index, "business_set"] = [
            "|".join(business_set_from_bin_set(pred_set)) for pred_set in test_prediction_sets
        ]
        ...
        per_fold_rows.append(
            {
                ...
                "selected_qhat": qhat,
                "selector_score": float(chosen_alpha["selector_score"]),
                **fold_metrics,
            }
        )
    ...
    summary = {
        ...
        "conformal_design": {
            ...
            "decision_rule": "ordered_interval_then_adjacent_overlap_to_warning_else_retest",
            "method": "ordered_interval_quantile_boundary_selector_warning_guardrail",
            "selector": config["conformal"]["selector"],
        },
        "data_summary": {
    ...
    lines = [
        ...
        "",
        "- Branch stage: conformal_ordered_interval_boundary_selector_warning_guardrail",
        "- Target: soft 5-bin distribution + ordered interval + boundary-aware selector + warning guardrail mapping",
    ...
    for row in per_fold_rows:
        lines.append(
            ...
            f"score={row['selector_score']:.4f}, coverage={row['hard_bin_coverage']:.4f}, retest={row['retest_rate']:.4f}"
        )

...
from run_centered_quality_center_band_current_head import (  # noqa: E402
    ...
    choose_semantic_candidate as _unused_old_selector,
    candidate_feature_columns_center_band,
    fit_binary_head,
    json_ready,
    risk_summary,
)
from run_centered_quality_current_head import decision_summary  # noqa: E402


DEFAULT_CONFIG_PATH = CENTERED_DIR / "configs" / "centered_quality_center_band_risk_guardrail_current_head.yaml"
    ...
    for clear_thr in grid_cfg["clear_risk_threshold_grid"]:
        for retest_thr in grid_cfg["retest_risk_threshold_grid"]:
            if float(retest_thr) >= float(clear_thr):
                continue
            for center_ceiling in grid_cfg["unacceptable_center_ceiling_grid"]:
                for premium_thr in grid_cfg["premium_center_threshold_grid"]:
                    if float(center_ceiling) >= float(premium_thr):
                        continue
                    rows.append(
                        {
                            "candidate_name": f"clear_{float(clear_thr):.2f}_retest_{float(retest_thr):.2f}_uceil_{float(center_ceiling):.2f}_premium_{float(premium_thr):.2f}",
                            "clear_risk_threshold": float(clear_thr),
                            "retest_risk_threshold": float(retest_thr),
                            "unacceptable_center_ceiling": float(center_ceiling),
...
def guardrail_decision(lower_risk: pd.Series, upper_risk: pd.Series, center_prob: pd.Series, candidate: dict[str, float | str]) -> pd.Series:
    max_risk = pd.concat([lower_risk.astype(float), upper_risk.astype(float)], axis=1).max(axis=1)
    center_prob = center_prob.astype(float)
    decision = np.select(
        [
            (max_risk >= float(candidate["clear_risk_threshold"])) & (center_prob <= float(candidate["unacceptable_center_ceiling"])),
...
def semantic_candidate_metrics(frame: pd.DataFrame, decision_col: str) -> dict[str, float]:
    ...
    return {
        ...
        "unacceptable_rate": unacceptable_rate,
    ...
    *,
    ...
    for candidate in candidates:
        ...
        for inner_train_index, inner_val_index in inner_tscv.split(train_frame):
            ...
            decision = guardrail_decision(lower_risk, upper_risk, center_prob, candidate)
    ...
    chosen = candidate_frame.sort_values(
        ["unacceptable_false_clear_rate", "unacceptable_recall", "macro_f1_vs_truth", "unacceptable_rate", "premium_precision", "retest_rate"],
...
def write_summary_report(path: Path, summary: dict[str, object]) -> None:
    lines = [
        "# Center-Band Risk-Guardrail Summary",
        "",
        "## Main outcome",
        "",
        f"- centered decision macro_f1: {summary['treatment_e_decision_summary']['macro_f1_vs_truth']:.4f}",
        f"- unacceptable recall: {summary['treatment_e_risk_summary']['unacceptable_recall']:.4f}",
        f"- unacceptable false-clear rate: {summary['treatment_e_risk_summary']['unacceptable_false_clear_rate']:.4f}",
        f"- premium precision: {summary['treatment_e_center_band_summary']['premium_precision_recall']['precision']:.4f}",
        f"- retest rate: {summary['treatment_e_decision_summary']['retest_rate']:.4f}",
    ]
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the center-band risk-guardrail calibration experiment.")
    ...
    for col in ["baseline_business_pred", "treatment_e_decision"]:
        predictions[col] = pd.NA
    for label in BUSINESS_LABELS:
        predictions[f"baseline_business_prob_{label}"] = pd.NA
    for col in ["treatment_e_lower_risk", "treatment_e_upper_risk", "treatment_e_center_band_prob"]:
        predictions[col] = pd.NA
    ...
    for fold, (train_index, test_index) in enumerate(outer_tscv.split(feature_rows), start=1):
        ...
        decision = guardrail_decision(lower_risk, upper_risk, center_prob, chosen_semantic_candidate)
        ...
        predictions.loc[test_frame.index, "treatment_e_lower_risk"] = lower_risk
        predictions.loc[test_frame.index, "treatment_e_upper_risk"] = upper_risk
        predictions.loc[test_frame.index, "treatment_e_center_band_prob"] = center_prob
        predictions.loc[test_frame.index, "treatment_e_decision"] = decision

        fold_scored = test_frame.copy()
        fold_scored["treatment_e_decision"] = decision
        ...
        per_fold_rows.append(
            {
                ...
                "unacceptable_center_ceiling": float(chosen_semantic_candidate["unacceptable_center_ceiling"]),
                ...
                "treatment_e_macro_f1_vs_truth": float(f1_score(fold_scored["centered_truth"], fold_scored["treatment_e_decision"], labels=CENTERED_LABELS, average="macro", zero_division=0)),
                "treatment_e_unacceptable_rate": float(fold_scored["treatment_e_decision"].eq("unacceptable").mean()),
                "treatment_e_retest_rate": float(fold_scored["treatment_e_decision"].eq("retest").mean()),
                "selected_sensor_scores": json.dumps(sensor_scores, ensure_ascii=False),
            }
        )

    for col in [c for c in predictions.columns if c not in {"sample_time", "business_label", "centered_truth", "baseline_business_pred", "treatment_e_decision"}]:
    ...
    treatment_e_risk_summary = risk_summary(scored_predictions, "treatment_e_lower_risk", "treatment_e_upper_risk", "treatment_e_decision")
    treatment_e_center_band_summary = center_band_summary(scored_predictions, "treatment_e_center_band_prob", "treatment_e_decision")
    treatment_e_decision_summary = decision_summary(scored_predictions, "treatment_e_decision")
    ...
    summary = {
        ...
        "treatment_e_risk_summary": treatment_e_risk_summary,
        "treatment_e_center_band_summary": treatment_e_center_band_summary,
        "treatment_e_decision_summary": treatment_e_decision_summary,
    ...
    feature_rows.to_csv(artifacts_dir / "centered_quality_center_band_risk_guardrail_feature_rows.csv", index=False, encoding="utf-8-sig")
    scored_predictions.to_csv(artifacts_dir / "centered_quality_center_band_risk_guardrail_results.csv", index=False, encoding="utf-8-sig")
    per_fold_frame.to_csv(artifacts_dir / "centered_quality_center_band_risk_guardrail_per_fold.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame(baseline_candidate_rows).to_csv(artifacts_dir / "centered_quality_center_band_risk_guardrail_baseline_candidate_search.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame(semantic_candidate_rows).to_csv(artifacts_dir / "centered_quality_center_band_risk_guardrail_semantic_candidate_search.csv", index=False, encoding="utf-8-sig")
    alias_frame.to_csv(artifacts_dir / "sensor_identity_alias_pairs.csv", index=False, encoding="utf-8-sig")
    (artifacts_dir / "centered_quality_center_band_risk_guardrail_summary.json").write_text(json.dumps(json_ready(summary), ensure_ascii=False, indent=2), encoding="utf-8")
    write_summary_report(reports_dir / "centered_quality_center_band_risk_guardrail_summary.md", json_ready(summary))

...
DEFAULT_CONFIG_PATH = CENTERED_DIR / "configs" / "centered_quality_center_band_two_tier_risk_retest_calibrated_current_head.yaml"
...
def write_summary_report(path: Path, summary: dict[str, object]) -> None:
    lines = [
        "# Center-Band Two-Tier Risk Retest-Calibrated Summary",
        "",
        "## Main outcome",
        "",
...
def main() -> None:
    ...
    feature_rows.to_csv(artifacts_dir / "centered_quality_center_band_two_tier_risk_retest_calibrated_feature_rows.csv", index=False, encoding="utf-8-sig")
    scored_predictions.to_csv(artifacts_dir / "centered_quality_center_band_two_tier_risk_retest_calibrated_results.csv", index=False, encoding="utf-8-sig")
    per_fold_frame.to_csv(artifacts_dir / "centered_quality_center_band_two_tier_risk_retest_calibrated_per_fold.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame(baseline_candidate_rows).to_csv(artifacts_dir / "centered_quality_center_band_two_tier_risk_retest_calibrated_baseline_candidate_search.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame(semantic_candidate_rows).to_csv(artifacts_dir / "centered_quality_center_band_two_tier_risk_retest_calibrated_semantic_candidate_search.csv", index=False, encoding="utf-8-sig")
    alias_frame.to_csv(artifacts_dir / "sensor_identity_alias_pairs.csv", index=False, encoding="utf-8-sig")
    (artifacts_dir / "centered_quality_center_band_two_tier_risk_retest_calibrated_summary.json").write_text(json.dumps(json_ready(summary), ensure_ascii=False, indent=2), encoding="utf-8")
    write_summary_report(reports_dir / "centered_quality_center_band_two_tier_risk_retest_calibrated_summary.md", json_ready(summary))

...
DEFAULT_CONFIG_PATH = CLEANROOM_DIR / "configs" / "ordinal_cumulative_paper_faithful_ewma_rep_specific_current_head.yaml"
...
def evaluate_representation(
    ...
    *,
    ...
    for fold_index, (train_index, test_index) in enumerate(tscv.split(frame), start=1):
        ...
        base_sensors, base_scores = select_topk_sensors(
            train_frame,
            baseline_feature_cols,
            topk=int(config["screening"]["topk_sensors"]),
        )
        ewma_sensors, ewma_scores = select_topk_sensors(
            train_frame,
            ewma_feature_cols,
            topk=int(config["screening"]["topk_sensors"]),
        )
        base_features = selected_feature_columns(baseline_feature_cols, base_sensors)
        ewma_features = selected_feature_columns(ewma_feature_cols, ewma_sensors)
        ...
        fold_truth = test_frame["business_label"]
        base_pred = probability_argmax(base_business, BUSINESS_LABELS)
        ewma_pred = probability_argmax(ewma_business, BUSINESS_LABELS)
        per_fold.append(
            {
                "fold": fold_index,
                ...
                "ewma_selected_sensor_count": int(len(ewma_sensors)),
                "sensor_overlap_count": int(len(set(base_sensors) & set(ewma_sensors))),
                "base_selected_sensors": base_sensors,
                "ewma_selected_sensors": ewma_sensors,
                "base_selected_sensor_scores": base_scores,
                "ewma_selected_sensor_scores": ewma_scores,
                "base_macro_f1": float(f1_score(fold_truth, base_pred, labels=BUSINESS_LABELS, average="macro", zero_division=0)),
                "ewma_macro_f1": float(f1_score(fold_truth, ewma_pred, labels=BUSINESS_LABELS, average="macro", zero_division=0)),
            }
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run EWMA with representation-specific sensor screening against the ordinal/cumulative baseline.")
    ...
    results_csv = artifacts_dir / "ordinal_cumulative_ewma_rep_specific_results.csv"
    summary_json = artifacts_dir / "ordinal_cumulative_ewma_rep_specific_summary.json"
    best_rows_csv = artifacts_dir / "ordinal_cumulative_ewma_rep_specific_best_feature_rows.csv"
    audit_md = reports_dir / "ordinal_cumulative_ewma_rep_specific_audit.md"
    ...
    summary = {
        ...
        "screening_policy": {
            "baseline_sensor_subset": "selected from baseline simple-window features inside each training fold",
            "ewma_sensor_subset": "selected from EWMA recursive features inside each training fold",
        },
    ...
    audit_lines = [
        "# Ordinal / Cumulative EWMA Representation-Specific Audit",
        "",
        ...
        "- Sensor selection change: EWMA now uses its own fold-local ranking instead of inheriting the baseline subset.",

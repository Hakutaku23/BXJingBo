...
DEFAULT_CONFIG = SCRIPT_DIR.parent / "configs" / "autogluon_centered_segmented_window_search.yaml"
    ...
    for record in sample_frame.itertuples(index=False):
        ...
        for segment_idx in range(segment_count):
            ...
            for key, value in raw_stats.items():
                ...
                if stat in stats:
                    feature_row[
                        f"{sensor}__lag{tau_minutes}_win{total_window_minutes}_seg{segment_idx + 1:02d}_len{segment_minutes}_{stat}"
                    ] = float(value)
...
def build_variant_feature_catalog(features: list[str], variant_name: str) -> pd.DataFrame:
    ...
    for column in features:
        if column.startswith("interaction__"):
            ...
            family = "segmented_lag_scale" if "_seg" in suffix else "whole_window_lag_scale"
        rows.append(
            {
...
def evaluate_variant(
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(splitter.split(frame), start=1):
        ...
        model_path = artifact_dir / f"ag_centered_segmented_{variant_name}_{run_id}_fold{fold_idx}"
...
def write_report(path: Path, summary_rows: list[dict[str, Any]], config: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "- The experiment only replaces the base window representation with segmented statistics",
        "",
        "## Fixed Settings",
        "",
        f"- lag_minutes: {config['snapshot']['lag_minutes']}",
        f"- total_window_minutes: {config['snapshot']['total_window_minutes']}",
        f"- top_k: {config['task']['top_k']}",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Centered-only segmented window search on top of the validated centered line.")
    ...
    min_points_per_segment = int(config["segmentation"]["min_points_per_segment"])
    ...
    summary["segment_minutes"] = None
    summary["segment_count"] = 1
    feature_catalog_parts.append(build_variant_feature_catalog(current_features, "current_whole_window_ref"))
    summary_rows.append(summary)

    for segment_minutes in config["segmentation"]["segment_minutes_candidates"]:
        segment_minutes = int(segment_minutes)
        segmented_snapshot = build_segmented_snapshot_table(
            labeled_samples=labeled,
            dcs=dcs,
            tau_minutes=tau_minutes,
        ...
        if segmented_snapshot.empty:
            continue
        segmented_frame = segmented_snapshot.merge(interaction_frame, on="decision_time", how="inner").merge(
            quality_table,
            on="decision_time",
            how="inner",
        )
        segmented_features = [column for column in segmented_frame.columns if "__" in column]
        variant_name = f"segmented_len_{segment_minutes:02d}"
        rows, summary = evaluate_variant(
            variant_name=variant_name,
            frame=segmented_frame,
            feature_columns=segmented_features,
            config=config,
            artifact_dir=artifact_dir,
            splitter=splitter,
            run_id=run_id,
        ...
        all_rows.extend(rows)
        summary["segment_minutes"] = segment_minutes
        summary["segment_count"] = int(total_window_minutes // segment_minutes)
        feature_catalog_parts.append(build_variant_feature_catalog(segmented_features, variant_name))
        summary_rows.append(summary)

    current_ref_mae = next(
        (float(row["autogluon_mean_mae"]) for row in summary_rows if row["variant_name"] == "current_whole_window_ref"),
        float("nan"),
    )
    for row in summary_rows:
        row["current_ref_autogluon_mae"] = current_ref_mae
    ...
    summary_rows = sorted(summary_rows, key=lambda item: (float(item["autogluon_mean_mae"]), str(item["variant_name"])))
    ...
    results_path = artifact_dir / "centered_segmented_window_search_results.csv"
    summary_path = artifact_dir / "centered_segmented_window_search_summary.json"
    catalog_path = artifact_dir / "centered_segmented_window_feature_catalog.csv"
    report_path = report_dir / "centered_segmented_window_search_summary.md"

    results_df.to_csv(results_path, index=False, encoding="utf-8-sig")
    summary_df.to_csv(artifact_dir / "centered_segmented_window_search_summary_rows.csv", index=False, encoding="utf-8-sig")

...
from run_ordinal_cumulative_current_head import format_threshold
...
DEFAULT_CONFIG_PATH = (
    CLEANROOM_DIR
    / "configs"
    / "ordinal_cumulative_current_head_monotonic_120min_boundary_weighted_inner_thresholds_identity_dedup_multiobjective.yaml"
)
...
def fit_cumulative_business_probabilities_inner_thresholds_only(
    train_frame: pd.DataFrame,
    ...
    *,
    ...
    candidate_weight = build_sample_weights(train_frame, candidate)
    cumulative = pd.DataFrame(index=test_frame.index)
    for threshold in thresholds:
        key = format_threshold(threshold)
        target_key = f"target_lt_{key}"
        sample_weight = candidate_weight if float(threshold) in weighted_thresholds else None
...
def add_selection_delta_columns(candidate_frame: pd.DataFrame, default_row: pd.Series) -> pd.DataFrame:
    enriched = candidate_frame.copy()
    enriched["macro_f1_delta_vs_default"] = enriched["macro_f1"] - float(default_row["macro_f1"])
    enriched["balanced_accuracy_delta_vs_default"] = enriched["balanced_accuracy"] - float(default_row["balanced_accuracy"])
    enriched["boundary_warning_average_precision_delta_vs_default"] = (
        enriched["boundary_warning_average_precision"] - float(default_row["boundary_warning_average_precision"])
    )
    enriched["clearly_unacceptable_average_precision_delta_vs_default"] = (
        enriched["clearly_unacceptable_average_precision"] - float(default_row["clearly_unacceptable_average_precision"])
    )
    enriched["boundary_high_confidence_non_warning_rate_delta_vs_default"] = (
        enriched["boundary_high_confidence_non_warning_rate"] - float(default_row["boundary_high_confidence_non_warning_rate"])
    )
    return enriched


def mark_selection_stage(frame: pd.DataFrame, stage_name: str) -> pd.DataFrame:
    marked = frame.copy()
    marked["selection_stage"] = stage_name
    return marked
...
def select_candidate_on_train(
    ...
    *,
    ...
    candidate_frame = add_selection_delta_columns(candidate_frame, default_row)

    objective_cfg = config["training_objective"]
    macro_tol = float(objective_cfg["inner_macro_f1_tolerance"])
    bal_tol = float(objective_cfg["inner_balanced_accuracy_tolerance"])
    unacc_tol = float(objective_cfg["inner_unacceptable_ap_tolerance"])
    boundary_tol = float(objective_cfg["inner_boundary_high_conf_non_warning_tolerance"])

    stage_macro_bal = candidate_frame[
    ...
    if stage_macro_bal.empty:
        stage_macro_bal = candidate_frame.loc[candidate_frame["candidate_name"] == "default"].copy()
    stage_macro_bal = mark_selection_stage(stage_macro_bal, "macro_bal_guardrail")

    stage_multiobjective = stage_macro_bal[
        (stage_macro_bal["clearly_unacceptable_average_precision"] >= float(default_row["clearly_unacceptable_average_precision"]) - unacc_tol)
        & (
            stage_macro_bal["boundary_high_confidence_non_warning_rate"]
            <= float(default_row["boundary_high_confidence_non_warning_rate"]) + boundary_tol
        )
    ].copy()

    if stage_multiobjective.empty:
        chosen_pool = mark_selection_stage(stage_macro_bal, "macro_bal_fallback")
    else:
        chosen_pool = mark_selection_stage(stage_multiobjective, "multiobjective_guardrail")

    chosen = chosen_pool.sort_values(
        [
            "boundary_high_confidence_non_warning_rate",
            "clearly_unacceptable_average_precision",
    ...
    audit_frame = pd.concat(
        [
            mark_selection_stage(candidate_frame, "all_candidates"),
            stage_macro_bal,
            chosen_pool,
        ],
        ignore_index=True,
    )
    return chosen.to_dict(), audit_frame


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Test inner-threshold-only boundary-aware weighting with an explicit multi-objective inner-CV selector."
    )
    ...
    for fold_index, (train_index, test_index) in enumerate(outer_tscv.split(feature_rows), start=1):
        ...
        selected_candidate_rows.append(
            {
                "fold": fold_index,
                "selection_stage": chosen_candidate["selection_stage"],
                **{k: v for k, v in chosen_candidate.items() if k != "selection_stage"},
            }
        )
        ...
        selected_features = selected_feature_columns(feature_cols, selected_sensors)
        default_cumulative, default_probabilities = fit_cumulative_business_probabilities_inner_thresholds_only(
            train_frame,
            test_frame,
            features=selected_features,
            thresholds=thresholds,
        ...
        weighted_cumulative, weighted_probabilities = fit_cumulative_business_probabilities_inner_thresholds_only(
            train_frame,
            test_frame,
            features=selected_features,
            thresholds=thresholds,
        ...
        per_fold_rows.append(
            {
                ...
                "mean_inner_threshold_train_weight": float(train_weights.mean()),
    ...
    per_fold_csv = artifacts_dir / "boundary_weighted_inner_thresholds_per_fold.csv"
    selected_candidate_csv = artifacts_dir / "boundary_weighted_inner_thresholds_selected_candidate_per_fold.csv"
    inner_search_csv = artifacts_dir / "boundary_weighted_inner_thresholds_inner_search.csv"
    results_csv = artifacts_dir / "boundary_weighted_inner_thresholds_results.csv"
    alias_csv = artifacts_dir / "sensor_identity_alias_pairs.csv"
    summary_json = artifacts_dir / "boundary_weighted_inner_thresholds_summary.json"
    report_md = reports_dir / "boundary_weighted_inner_thresholds_summary.md"
    ...
    summary = {
        ...
        "selection_policy": {
            "macro_f1_tolerance": float(config["training_objective"]["inner_macro_f1_tolerance"]),
            "balanced_accuracy_tolerance": float(config["training_objective"]["inner_balanced_accuracy_tolerance"]),
            "unacceptable_ap_tolerance": float(config["training_objective"]["inner_unacceptable_ap_tolerance"]),
            "boundary_high_conf_non_warning_tolerance": float(
                config["training_objective"]["inner_boundary_high_conf_non_warning_tolerance"]
            ),
            "sort_priority": [
                "boundary_high_confidence_non_warning_rate asc",
                "clearly_unacceptable_average_precision desc",
                "boundary_warning_average_precision desc",
                "macro_f1 desc",
    ...
    lines = [
        "# Boundary-Weighted Inner Thresholds Multiobjective Summary",
        "",
        ...
        "- Intervention: inner-threshold-only hard weighting with explicit multi-objective inner-CV candidate selection",
        ...
        "## Selection Policy",
        "",
        f"- macro_f1 tolerance: {config['training_objective']['inner_macro_f1_tolerance']}",
        f"- balanced_accuracy tolerance: {config['training_objective']['inner_balanced_accuracy_tolerance']}",
        f"- unacceptable_AP tolerance: {config['training_objective']['inner_unacceptable_ap_tolerance']}",
        f"- boundary high-confidence non-warning tolerance: {config['training_objective']['inner_boundary_high_conf_non_warning_tolerance']}",
        "",
    ...
    for row in selected_candidate_rows:
        lines.append(
            ...
            f"(stage={row['selection_stage']}, boundary_weight={row['boundary_weight']}, warning_weight={row['warning_weight']})"
        )

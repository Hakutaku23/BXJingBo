...
DEFAULT_CONFIG = SCRIPT_DIR.parent / "configs" / "autogluon_soft_target_centered_x_ablation_round2.yaml"


def build_ablation_frames(config_path: Path, config: dict[str, Any]) -> dict[str, pd.DataFrame]:
    ...
    quality_feature_columns = [column for column in quality_table.columns if column.startswith("quality__")]
    interaction_feature_columns = list(interaction_frame.columns)
    ...
    quality_only = quality_merged[quality_feature_columns].reset_index(drop=True)
    interaction_only = interaction_frame.reset_index(drop=True)

    frames = {
        "lag_only": base_meta.copy(),
        "lag_plus_interaction": pd.concat([base_meta.reset_index(drop=True), interaction_only], axis=1),
        "lag_plus_quality": pd.concat([base_meta.reset_index(drop=True), quality_only], axis=1),
        "lag_plus_interaction_plus_quality": pd.concat(
            [base_meta.reset_index(drop=True), interaction_only, quality_only], axis=1
        ),
    }
    return {
...
def evaluate_variant(
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(splitter.split(frame), start=1):
        ...
        model_path = artifact_dir / f"ag_soft_target_centered_x_ablation_{variant_name}_{run_id}_fold{fold_idx}"
...
def write_report(path: Path, summary_rows: list[dict[str, Any]]) -> None:
    lines = [
        "# Soft Target On Centered-X Ablation Round 2",
        "",
        "## Scope",
        "",
        "- Freeze the current strongest pair from controlled round 1: `soft_target + centered_x_only`.",
        "- Decompose `centered_x_only` into confirmatory ablation variants.",
        "- Keep model family, split rule, and top-k budget fixed.",
        "",
        "## Summary Rows",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run confirmatory ablation for soft_target on centered-X packages.")
    ...
    frames = build_ablation_frames(config_path, config)
    ...
    summary_path = artifact_dir / "soft_target_centered_x_ablation_round2_summary.json"
    results_path = artifact_dir / "soft_target_centered_x_ablation_round2_results.csv"
    report_path = report_dir / "soft_target_centered_x_ablation_round2_summary.md"

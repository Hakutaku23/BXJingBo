...
DEFAULT_CONFIG_PATH = CLEANROOM_DIR / "configs" / "ordinal_cumulative_current_head_monotonic_120min_ablation_v2.yaml"
...
def main() -> None:
    ...
    dcs = load_dcs_data(PROJECT_DIR / str(baseline_config["data"]["dcs_path"]))
    ...
    for row in baseline_summary["metrics_summary"]["per_fold"]:
        selected_counter.update(row["selected_sensors"])
